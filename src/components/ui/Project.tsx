    ...
    >
      ...
      <div className="relative w-full h-[230px] sm:h-[320px] lg:h-[360px] mb-4 rounded-lg overflow-hidden">
        {imageUrl ? (
          ...
          <div className={`w-full h-full bg-red-500 flex items-center justify-center transition-transform duration-300 ${hasLink ? 'group-hover:scale-105' : ''}`}>
            <span className="text-white font-medium text-sm opacity-50">IMAGE</span>
          </div>
        )}
        
        {/* Overlay hover */}
        <div className={`absolute inset-0 bg-black/0 transition-all duration-300 ${hasLink ? 'group-hover:bg-black/20' : ''}`} />
      ...
      <div className="flex flex-col mb-2">
        <h3 className={`font-medium text-[20px] lg:text-[18px] 2xl:text-[20px] transition-colors duration-300 ${
          hasLink 
