...
export default function FounderCard({ data, className = "" }: FounderCardProps) {
  return (
    <div
      className={`flex w-[300px] items-center gap-4 border border-white/10 bg-shark p-2 backdrop-blur-[2px] ${className}`}
    >
      <div className="relative h-[120px] w-[95px] shrink-0 overflow-hidden border border-white/10 bg-white/5">
        <Image
          src={data.image}
          alt={data.name}
          fill
      ...
      <div className="flex h-[120px] min-w-0 flex-1 flex-col justify-between py-2">
        ...
        <BookingButton
          ...
          className="text-fig-link inline-flex h-9 w-full items-center justify-center gap-2.5 border border-white/10 bg-white px-4 text-black transition-colors hover:bg-white/90"
        />
      </div>
    </div>
  );
