 ...
 * Matches the design spec: 8px rounded card on Shark bg, +/− icon (Silver) that
...
export default function HomeFaqItem({ question, answer, defaultOpen = false }: HomeFaqItemProps) {
  ...
  return (
    <div className="bg-shark overflow-hidden border border-white/10">
      <button
        type="button"
        onClick={() => setIsOpen((v) => !v)}
        aria-expanded={isOpen}
        className="flex w-full cursor-pointer items-center justify-between gap-4 p-6 text-left md:p-7"
      >
        ...
        <span className="text-silver relative flex h-6 w-6 shrink-0 items-center justify-center">
          <span aria-hidden="true" className="absolute inset-x-1 mx-auto block h-[2px] bg-current" />
          <span
            ref={verticalBarRef}
            aria-hidden="true"
            className="absolute inset-y-1 my-auto block h-4 w-[2px] origin-center bg-current"
      ...
      >
        <p ref={answerRef} className="text-fig-body text-silver max-w-[640px] px-6 pb-6 md:px-7 md:pb-7">
          {answer}
        </p>
      </div>
    </div>
